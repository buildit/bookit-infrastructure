if [[ $# -lt 2 ]] ; then
  echo "Usage: protect-riglet <prefix> <enable|disable|list> "
...
list_stacks() {
  aws cloudformation list-stacks --stack-status CREATE_COMPLETE UPDATE_COMPLETE | \
...
for stack in $(list_stacks ${prefix}); do
  case "${mode}" in
    enable) echo "Enabling protection: ${stack} "; aws cloudformation update-termination-protection --enable-termination-protection --stack-name $stack;
            ;;
    disable) echo "Disabling protection ${stack}"; aws cloudformation update-termination-protection --no-enable-termination-protection --stack-name $stack;
             ;;
    list) echo ${stack}
         ;;
    *) echo "Invalid mode: ${mode} for ${stack}"
  esac
done
